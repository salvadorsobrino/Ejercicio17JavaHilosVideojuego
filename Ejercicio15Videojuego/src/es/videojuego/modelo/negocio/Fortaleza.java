...
public class Fortaleza {
	 ...
	 * cual se le irá restando vida 
	 * @param personaje le restará vida al Personaje Jefe
	 */
	public synchronized void entrar(Personaje personaje) { 
		if (jefe.getVida()>0) {
			System.out.println(personaje.getNombre() +" entró en la Fortaleza " + this.nombre +" y luchará contra "+jefe.getNombre());
			Batalla batalla = new Batalla();
			batalla.setP1(jefe);
			batalla.setP2(personaje);
			Personaje pGanador = batalla.combatir();
			System.out.println(pGanador +" salió victorioso");
			if(jefe.getVida()<=0) {
				campeon = pGanador;
			} else {
				System.out.println("¿Quién podrá derrotar a "+jefe.getNombre()+"?");
			}
		} else {
